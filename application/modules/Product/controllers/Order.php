					...
					if(is_array($order) AND !empty($order)){
						if($order['status']>0){
							...
							try{
								$payclass = "\\Pay\\".$paymethod."\\".$paymethod;
								$PAY = new $payclass();
								$params =array('orderid'=>$orderid,'quantity'=>$order['number'],'email'=>$order['email'],'money'=>$order['money'],'productname'=>$order['productname'],'weburl'=>$this->config['weburl'],'order'=>$order);
